...
4. **Verify v86 is cloned**: Check that `v86-workdir/` contains the v86 source. If not, tell the human to run `uv run prepare.py --clone-v86`.
5. **Verify baseline exists**: Check that `baseline/` contains `baseline_video.mp4` and `baseline_frames/`. If not, tell the human to run `uv run prepare.py`.
...
Each experiment runs the emulation benchmark for a **fixed time budget of 5 seconds** (wall clock). You launch it via: `uv run emulate.py`.
...
**The first run**: Your very first run should always establish the baseline by running `uv run emulate.py` with the unmodified `jit.rs`.
...
```bash
uv run evo_db.py add --commit <hash, short, 7 chars> --parent <id> --description "..." --log run.log
```

**Recording a crashed experiment (build failure, runtime crash, etc.):**
```bash
uv run evo_db.py add-crash --commit <hash, short, 7 chars> --parent <id> --description "..."
```

**Other useful commands:**
```bash
uv run evo_db.py sample    # Get next parent + inspirations (JSON)
uv run evo_db.py status    # Population overview with MAP-Elites grids
uv run evo_db.py best      # Show best experiment
uv run evo_db.py history   # Recent experiments
```
...
6. **RUN**: `uv run emulate.py > run.log 2>&1`
7. **RECORD**:
   ...
   - If success: `uv run evo_db.py add --commit <hash> --parent <parent_id> --description "..." --log run.log`
   - If crash: `uv run evo_db.py add-crash --commit <hash> --parent <parent_id> --description "..."`
8. Optionally: `uv run evo_db.py status` to review population state.
9. **GOTO 1**
